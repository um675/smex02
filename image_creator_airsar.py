def image_creator_airsar(rasterfile,classificata,classificata2,num,pos_out,out_name):
    import numpy as np
    import gdal
    import os
    import math
    for iii in range(0,num):
        ...
        classificata2_array = classificata2.ReadAsArray(0, 0, classificata2.RasterXSize, classificata2.RasterYSize)
        ...
        indclass21 = np.nonzero(classificata2_array == 1)
        indclass22 = np.nonzero(classificata2_array == 2)
        indclass23 = np.nonzero(classificata2_array == 3)
        indclass24 = np.nonzero(classificata2_array == 4)
        indclass25 = np.nonzero(classificata2_array == 5)
        indclass26 = np.nonzero(classificata2_array == 6)
        indclass27 = np.nonzero(classificata2_array == 7)
        indclass28 = np.nonzero(classificata2_array == 8)
        indclass29 = np.nonzero(classificata2_array == 9)
        indclass210 = np.nonzero(classificata2_array == 10)

        indlist = [indclass1, indclass2, indclass3, indclass4, indclass5, indclass6, indclass7, indclass8, indclass9,
                   indclass10]
        indlist2 = [indclass21, indclass22, indclass23, indclass24, indclass25, indclass26, indclass27, indclass28,
                    indclass29,
                    indclass210]
        ...
        for i in range(0, 10):
            for jj in range(0, (rasterfile_nbands)):
                ...
                if jj < 4:
                    ...
                    rasterfile_band = rasterfile.GetRasterBand(jj + 1)
                    rasterfile_array = rasterfile_band.ReadAsArray(0, 0, rasterfile.RasterXSize, rasterfile.RasterYSize)
                    matsd[i, jj] = np.std(rasterfile_array[indlist2[i]])
                    if math.isnan(matsd[i, jj]):
                        matsd[i, jj] = 999
            ...
            for i in range(0, rasterfile.RasterYSize):
                for j in range(0, rasterfile.RasterXSize):
                    ...
                    if (k + 1) < 5:
                        ...
                        classe = classificata2_array[i, j]
                    # print classe , "classe"
                    sd = matsd[classe - 1, k]
